...
//    console.log("intent: TEXT");
    g_conversation=conv;
    g_input=input;
    ws_context=conv.data && conv.data.ws_context ? conv.data.ws_context : initialContext;
    assistant.message({
        workspace_id: workspace_id,
        input: { text: g_input },
        context: ws_context,
...
//      console.log("error: ",err); // something went wrong      
      return g_conversation.close('Sorry, an error occurred. Goodbye!');
    }
    else { // all ok
...
//                response.context.ispezione_iniziata=1;
                    ...
                    return g_conversation.close('Goodbye!');
                }
            }
        }
        else { // finish
        ...
        g_conversation.data.ws_context=response.context;        
        g_conversation.ask(frase_da_pronunciare);
    }
}
